...
\item \emph{profesor} podatke o predmetih, ki jih predava. Hkrati moramo za vsako uro v tednu vedeti, če
...
\item \emph{Skupina} predstavlja skupino študentov. Kot podatke torej potrebujemo
...
\item\emph{Predavalnica} nosi podatke o kapaciteti predavalnice, njeno oznako
   ter podatke o zasedenosti, ki so predstavljeni s tabelo.

\item\emph{Predmet} vsebuje podatke o posameznih predmetih. Kot podatke tega
...
kakovost urnika:
$$ k_1 \cdot u_1 + k2 \cdot u_2 + \cdots + k_{n-1} \cdot u_{n-1} + k_n \cdot u_n $$
...
\begin{enumerate}
   ...
   $j$-tem koraku označimo s $_j$.

   \item Za vse proste termine predavalnice $_j$ izračunamo ceno vstavljanja srečanja
   $_i$, ki ga sestavlja predmet $_i$, profesor, ki ta predmet predava,
   ...
   \item Ko preverimo vse proste termine vseh predavalnic, vstavimo srečanje $_i$
...
spremembo (npr. zamenjava $_i$ in $_j$) sedaj shranimo v
...
\begin{enumerate}
   ...
   \item Rešitev $_r$, ki smo jo dobili s pomočjo požrešne metode, izboljšamo z lokalnim vzpenjanjem
   ...
   \item Na vsakem koraku ocenimo izboljšamo rešitev $_r$. Če je vrednost kriterijske funkcije
   za trenutno rešitev prešla zastavljeno zadovoljivo mejo, potem se program zaključi. Prav tako
   se zaključi, če po $_n$ korakih še vedno tičimo v istem lokalnem maksimumu.
\end{enumerate}
%
